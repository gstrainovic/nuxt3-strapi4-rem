            ...
            <div class="faq__tab tp-tab pr-200">
              <nav>
                <div class="nav nav-tabs flex-column" id="nav-tab" role="tablist">
                  <button v-for="(item, index) in accordionItems" :key="index"
                    :class="'nav-link' + (index === 0 ? ' active' : '')" :id="'nav-' + item.id + '-tab'"
                    :data-bs-toggle="'tab'" :data-bs-target="'#nav-' + item.id" :type="'button'" :role="'tab'"
                    :aria-controls="'nav-' + item.id" :aria-selected="index === 0">
                    <span>
          ...
          <div class="faq__tab-content tp-accordion">
            <div class="tab-content" :id="'nav-tabContent-' + activeTab">
              <div v-for="(item, index) in accordionItems" :key="index"
                :class="'tab-pane fade' + (index === 0 ? ' show active' : '')" :id="'nav-' + item.id" :role="'tabpanel'"
                  ...
                  <div v-for="(subItem, subIndex) in item.subItems" :key="subIndex" class="accordion-item">
                    <h2 class="accordion-header" :id="item.id + '_' + subIndex">
                      <button class="accordion-button" type="button" :data-bs-toggle="'collapse'"
                        :data-bs-target="'#' + item.id + '_' + subIndex + '_collapse'" :aria-expanded="subIndex === 0"
                        :aria-controls="item.id + '_' + subIndex + '_collapse'">
                        {{ subItem.title }}
                      </button>
                    </h2>
                    <div :id="item.id + '_' + subIndex + '_collapse'" class="accordion-collapse collapse"
                      :class="subIndex === 0 ? 'show' : ''" :aria-labelledby="item.id + '_' + subIndex"
                      :data-bs-parent="'#' + item.id + '_accordion'">
